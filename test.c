...
XContext setup(Display * dpy, int argc, char ** argv){
	...
	mainwindow->background = getColour(dpy,  db, "xtut9.background", "xtut9.BackGround", "DarkBlue");
	...
	createButton(dpy, win, "Exit", font, /*display text font */
		mainwindow->width/2-180, 17, 80, (font->ascent+font->descent)*2,/*xywh*/
			/* colours */
...
int main_loop(Display *dpy, XContext context){
	...
	while(1){
        ...
        XFindContext(ev.xany.display, ev.xany.window, context, (XPointer*)&button1);
		...
		case EnterNotify:
            ...
            if (button1)
				buttonEnter(button1, &ev);
			break;
		case LeaveNotify:
			if (button)
				buttonLeave(button, &ev);
            if (button1)
				buttonLeave(button1, &ev);
			break;
		case ButtonRelease:
			if (button && button->buttonRelease)
				button->buttonRelease(button1->cbdata);
            if (button1 && button1->buttonRelease)
				button1->buttonRelease(button1->cbdata);
			break;
		}
	}
}
